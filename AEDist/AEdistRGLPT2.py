...
# -*- coding: utf-8 -*-
"""
This experiment was created using PsychoPy2 Experiment Builder (v1.79.01), 2014_05_13_1215
...
#Verze z kvetna 2014 
#- pridana pauza pred trialem, s promennou delkou ISItime = random()/2+0.5
#- pridan fixaxni krizek behem pauzy
#- zde jeste pridelana podpora paralelniho portu
...
expName = 'AEdistRG2'  # from the Builder filename that created this script
...
# Setup the Window
# kamil - nebo screen=1, size=(1920, 1080), obrazky maj sirku 1839
...
trials = data.TrialHandler(nReps=1, method='sequential', 
    ...
    trialList=data.importConditions('AEDistRG.csv'),
...
for thisTrial in trials:
    ...
    while continueRoutine:
        ...
        # *test_cross* updates
        if t >= 0.0 and test_cross.status == NOT_STARTED:
            # keep track of start time/frame for later
        ...
        elif test_cross.status == STARTED and t >= (0.0 + ISItime):
            test_cross.setAutoDraw(False)
        
        # *image* updates
        if t >= ISItime and image.status == NOT_STARTED:
            # keep track of start time/frame for later
        ...
        # *odpoved* updates
        if t >= ISItime and odpoved.status == NOT_STARTED:
            # keep track of start time/frame for later
