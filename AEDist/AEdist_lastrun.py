...
# -*- coding: utf-8 -*-
"""
This experiment was created using PsychoPy2 Experiment Builder (v1.79.01), leden 28, 2014, at 10:07
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath=u'D:\\prace\\programovani\\psychopy\\AEDist\\AEdist.psyexp',
...
textPauza = visual.TextStim(win=win, ori=0, name='textPauza',
    text=u'Zm\xe1\u010dkn\u011bte mezern\xedk pro pokra\u010dov\xe1n\xed',    font='Arial',
...
trials = data.TrialHandler(nReps=1, method='sequential', 
    extraInfo=expInfo, originPath=u'D:\\prace\\programovani\\psychopy\\AEDist\\AEdist.psyexp',
    trialList=data.importConditions('AEDist.csv'),
...
for thisTrial in trials:
    ...
    for thisLoopPauza in loopPauza:
        ...
        respPauza = event.BuilderKeyResponse()  # create an object of type KeyResponse
        respPauza.status = NOT_STARTED
        # keep track of which components have finished
        pauzaComponents = []
        pauzaComponents.append(textPauza)
        pauzaComponents.append(respPauza)
        ...
        while continueRoutine:
            ...
            # *textPauza* updates
            if t >= 0.0 and textPauza.status == NOT_STARTED:
                # keep track of start time/frame for later
                textPauza.tStart = t  # underestimates by a little under one frame
                textPauza.frameNStart = frameN  # exact frame index
                textPauza.setAutoDraw(True)
            
            # *respPauza* updates
            if t >= 0.0 and respPauza.status == NOT_STARTED:
                # keep track of start time/frame for later
                respPauza.tStart = t  # underestimates by a little under one frame
                respPauza.frameNStart = frameN  # exact frame index
                respPauza.status = STARTED
                # keyboard checking is just starting
                respPauza.clock.reset()  # now t=0
                event.clearEvents()
            if respPauza.status == STARTED:
                theseKeys = event.getKeys(keyList=['space'])
                if len(theseKeys) > 0:  # at least one key was pressed
                    respPauza.keys = theseKeys[-1]  # just the last key pressed
                    respPauza.rt = respPauza.clock.getTime()
        ...
        # check responses
        if respPauza.keys in ['', [], None]:  # No response was made
           respPauza.keys=None
        # store data for loopPauza (TrialHandler)
        loopPauza.addData('respPauza.keys',respPauza.keys)
        if respPauza.keys != None:  # we had a response
            loopPauza.addData('respPauza.rt', respPauza.rt)
        thisExp.nextEntry()
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        elif text.status == STARTED and t >= (0.5 + 2.0):
    ...
    for thisLoopFeedback in loopFeedback:
        ...
        while continueRoutine and routineTimer.getTime() > 0:
            ...
            elif textFeedBack.status == STARTED and t >= (0.0 + 1.0):
