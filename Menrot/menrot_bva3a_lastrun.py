...
# -*- coding: utf-8 -*-
"""
This experiment was created using PsychoPy2 Experiment Builder (v1.79.01), duben 30, 2014, at 12:31
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='D:\\prace\\programovani\\psychopy\\Menrot\\menrot_bva3a.psyexp',
...
LPT = parallel.ParallelPort(address='0x03BC')
...
trials = data.TrialHandler(nReps=1, method='sequential', 
    extraInfo=expInfo, originPath='D:\\prace\\programovani\\psychopy\\Menrot\\menrot_bva3a.psyexp',
    trialList=data.importConditions('menrot_bva3A.csv'),
    ...
    while continueRoutine:
        ...
        if resp.status == STARTED:
            ...
            if len(theseKeys) > 0:  # at least one key was pressed
                ...
                continueRoutine = False
        # *LPT* updates
        if t >= 1 and LPT.status == NOT_STARTED:
            # keep track of start time/frame for later
            LPT.tStart = t  # underestimates by a little under one frame
            LPT.frameNStart = frameN  # exact frame index
            LPT.status = STARTED
            win.callOnFlip(LPT.setData, int(255))
        elif LPT.status == STARTED and t >= (1 + 2):
            LPT.status = STOPPED
            win.callOnFlip(LPT.setData, int(0))
        # *ISI* period
